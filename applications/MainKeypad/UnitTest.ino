...
#include <esp_sntp.h>
#include "src/cws_LOGO.c"
LV_IMG_DECLARE(CWS_Logo);
...
// Touch state isolation for menu transitions
static bool touch_state_isolated = false;
static uint32_t touch_isolation_start = 0;
static uint32_t current_screen_id = 0;
...
// Touch state isolation functions
void isolate_touch_input(uint32_t screen_id);
bool is_touch_isolated();
...
void loopGPS()
{
    ...
    // Time-slice GPS processing: only run every 50ms to prevent UI blocking
    if (current_time - last_gps_time < 50) {
        return;
    }
    ...
    do {
        // Process only a limited amount of GPS data per cycle
        int bytes_processed = 0;
        while (SerialGPS.available() && bytes_processed < 32) {
        ...
        // Yield to LVGL every 5ms during GPS processing
        if (millis() - start >= 5) {
            lv_task_handler();
            break;
        }
    } while (millis() - start < 15); // Reduced from 20ms to 15ms
...
void loopRadio()
{
    ...
    // Time-slice radio processing: only run every 30ms to prevent UI blocking
    if (current_time - last_radio_time < 30) {
        return;
    }
    ...
    if ( xSemaphoreTake( xSemaphore, ( TickType_t ) 10 ) == pdTRUE ) { // Reduced timeout from portMAX_DELAY to 10ms
        digitalWrite(BOARD_SDCARD_CS, HIGH);
        digitalWrite(RADIO_CS_PIN, HIGH);
        digitalWrite(BOARD_TFT_CS, HIGH);
...
void setup()
{
    ...
    // Disable scrollbars on the main screen to remove scroll indicators
    lv_obj_clear_flag(lv_scr_act(), LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scrollbar_mode(lv_scr_act(), LV_SCROLLBAR_MODE_OFF);

    // Show wallpaper image as background
    lv_obj_t *logo_img = lv_img_create(lv_scr_act());
    lv_img_set_src(logo_img, &CWS_Logo); // Provide both arguments: object and image descriptor pointer
    lv_obj_align(logo_img, LV_ALIGN_CENTER, 0, -30);

    // Create software information panel below the logo (transparent)
    lv_obj_t *info_panel = lv_obj_create(lv_scr_act());
    lv_obj_set_size(info_panel, LV_PCT(80), LV_SIZE_CONTENT);
    lv_obj_set_style_bg_opa(info_panel, LV_OPA_TRANSP, LV_PART_MAIN);  // Make background transparent
    lv_obj_set_style_border_width(info_panel, 0, LV_PART_MAIN);        // Remove border
    lv_obj_set_style_pad_all(info_panel, 12, LV_PART_MAIN);
    lv_obj_align(info_panel, LV_ALIGN_CENTER, 0, 80);
    
    // Disable scrolling and scrollbars on the info panel
    lv_obj_clear_flag(info_panel, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scrollbar_mode(info_panel, LV_SCROLLBAR_MODE_OFF);
    
    // Create software info text
    lv_obj_t *info_label = lv_label_create(info_panel);
    
    // Create firmware name with larger, bold font
    lv_obj_t *firmware_label = lv_label_create(info_panel);
    lv_label_set_text(firmware_label, "Citadel ADS One Keypad v.1");
    lv_obj_set_style_text_color(firmware_label, lv_color_hex(0xFFFFFF), LV_PART_MAIN);
    lv_obj_set_style_text_font(firmware_label, &lv_font_montserrat_16, LV_PART_MAIN);  // Larger font
    lv_obj_set_style_text_align(firmware_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
    lv_obj_align(firmware_label, LV_ALIGN_TOP_MID, 0, 0);
    
    // Create developer credit with professional styling
    lv_obj_t *dev_label = lv_label_create(info_panel);
    lv_label_set_text(dev_label, "Developed by Aaron Escobar");
    lv_obj_set_style_text_color(dev_label, lv_color_hex(0x9CDCFE), LV_PART_MAIN);  // Subtle blue accent
    lv_obj_set_style_text_font(dev_label, &lv_font_montserrat_12, LV_PART_MAIN);   // Medium size
    lv_obj_set_style_text_align(dev_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
    lv_obj_align(dev_label, LV_ALIGN_TOP_MID, 0, 22);  // Position below firmware name
    
    // Create other software info with normal font
    String software_info = String("LVGL v") + lv_version_major() + "." + lv_version_minor() + "." + lv_version_patch() + "\n" +
                          "Arduino ESP32 v" + ESP_ARDUINO_VERSION_MAJOR + "." + ESP_ARDUINO_VERSION_MINOR + "." + ESP_ARDUINO_VERSION_PATCH + "\n" +
                          "TFT_eSPI v2.5.22\n" +
                          "Build: " + String(__DATE__) + " " + String(__TIME__);
    
    lv_label_set_text(info_label, software_info.c_str());
    lv_obj_set_style_text_color(info_label, lv_color_hex(0xCCCCCC), LV_PART_MAIN);  // Slightly dimmer for other info
    lv_obj_set_style_text_font(info_label, &lv_font_montserrat_12, LV_PART_MAIN);   // Small font
    lv_obj_set_style_text_align(info_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);
    lv_obj_align(info_label, LV_ALIGN_TOP_MID, 0, 45);  // Position below developer credit

    // Show logo and info with fade-in effect
    for (int i = 0; i <= 16; ++i) {
        setBrightness(i);
        lv_timer_handler();
        delay(30);
    ...
    // Process any pending LVGL tasks before transitioning
    lv_task_handler();
    ...
    // Clean up splash screen before showing main UI
    lv_obj_del(logo_img);
    lv_obj_del(info_panel);

    // Clear the screen to ensure clean transition
    lv_obj_clean(lv_scr_act());

    setupUI();

    // Create professional arrow cursor using built-in LVGL symbol
    lv_obj_t *cursor_obj = lv_label_create(lv_scr_act());
    lv_label_set_text(cursor_obj, LV_SYMBOL_GPS);  // Use GPS symbol which looks like an arrow pointer
    lv_obj_set_style_text_color(cursor_obj, lv_color_hex(0xFFFFFF), LV_PART_MAIN);  // White cursor
    lv_obj_set_style_text_font(cursor_obj, &lv_font_montserrat_12, LV_PART_MAIN);   // Appropriate size
    lv_obj_set_style_bg_opa(cursor_obj, LV_OPA_TRANSP, LV_PART_MAIN);  // Transparent background
    lv_obj_set_style_border_width(cursor_obj, 0, LV_PART_MAIN);        // No border
    lv_obj_set_style_pad_all(cursor_obj, 0, LV_PART_MAIN);             // No padding
    
    lv_indev_set_cursor(mouse_indev, cursor_obj);           /*Connect the cursor to the driver*/

    // Force LVGL to process the display updates and show the home screen
    for (int i = 0; i < 10; i++) {
        lv_task_handler();
        delay(5);
    }
...
void loop()
{
    ...
    // Optimize for scroll smoothness: process LVGL more frequently with minimal delay
    lv_task_handler();
    
    // Reduced delay for smoother scrolling and UI responsiveness
    delay(1); // 1ms delay = ~1000 FPS max, ultra-smooth scrolling
}
...
static void mouse_read(lv_indev_drv_t *indev, lv_indev_data_t *data)
{
    static int16_t last_x = 160; // Start in center
    static int16_t last_y = 120; // Start in center
    static uint32_t last_read_time = 0;
    static bool last_dir[5] = {false, false, false, false, false};
    static uint32_t last_change_time[5] = {0, 0, 0, 0, 0};
    
    bool left_button_down = false;
    uint32_t current_time = millis();
    
    // Debounce timing - reduce from default to make more responsive
    const uint32_t debounce_delay = 20; // 20ms debounce instead of default
    
    // Skip reading if called too frequently to prevent lag
    if (current_time - last_read_time < 3) { // Reduced from 5ms to 3ms for ultra-responsive scrolling
        data->point.x = last_x;
        data->point.y = last_y;
    ...
    // Increased movement speed for more responsive feel
    uint8_t pos = 15; // Increased from 10 to 15 for faster movement
    
    for (int i = 0; i < 5; i++) {
        bool dir = digitalRead(dir_pins[i]);
        
        // Only process if state changed and debounce time has passed
        if (dir != last_dir[i] && (current_time - last_change_time[i]) > debounce_delay) {
            last_dir[i] = dir;
            last_change_time[i] = current_time;
            
            // Only move on button press, not release for cleaner movement
            if (dir == LOW) { // Button pressed (assuming active low)
                switch (i) {
                case 0: // Right
                    {
                        int16_t new_x = last_x + pos;
                        int16_t max_x = lv_disp_get_hor_res(NULL) - 20;
                        last_x = (new_x < max_x) ? new_x : max_x;
                    }
                    break;
                case 1: // Up
                    {
                        int16_t new_y = last_y - pos;
                        last_y = (new_y > 20) ? new_y : 20;
                    }
                    break;
                case 2: // Left
                    {
                        int16_t new_x = last_x - pos;
                        last_x = (new_x > 20) ? new_x : 20;
                    }
                    break;
                case 3: // Down
                    {
                        int16_t new_y = last_y + pos;
                        int16_t max_y = lv_disp_get_ver_res(NULL) - 20;
                        last_y = (new_y < max_y) ? new_y : max_y;
                    }
                    break;
                case 4: // Click
            ...
            // Keep button pressed if still held down
            left_button_down = true;
        }
    }
...
static void touchpad_read( lv_indev_drv_t *indev_driver, lv_indev_data_t *data )
{
    ...
    // Check if touch input should be isolated during screen transitions
    if (is_touch_isolated()) {
        // During isolation period, only allow touch release
        if (!touch.isPressed()) {
            touch_state_isolated = false; // Reset isolation when touch is released
        }
        return; // Block all touch input during isolation
    }
    
    // Optimize touch reading for smoother scrolling
    if (touch.isPressed()) {
    ...
    // Add small hysteresis for more stable touch detection during scrolling
    if (current_time - last_touch_time < 10 && was_pressed) { // 10ms hysteresis
        // Keep last valid position for smoother scrolling transitions
...
static void setupLvgl()
{
    ...
    // Don't create cursor during LVGL setup - it will interfere with splash screen
    // Cursor will be created after splash screen cleanup
...
// Touch state isolation functions to prevent cross-menu button activation
void isolate_touch_input(uint32_t screen_id) 
{
    touch_state_isolated = true;
    touch_isolation_start = millis();
    current_screen_id = screen_id;
}

bool is_touch_isolated()
{
    if (!touch_state_isolated) {
        return false;
    }
    
    uint32_t current_time = millis();
    if (current_time - touch_isolation_start >= TOUCH_ISOLATION_TIME) {
